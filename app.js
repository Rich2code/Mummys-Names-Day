...
async function createCard(data, index) {
  ...
  el.innerHTML = `<img src="${data.src}" alt="${data.alt}" draggable="false" /><span class="caption">${data.caption}</span>`;
  stage.appendChild(el);

  const speed = reducedMotion ? 0 : 18 + Math.random() * 16;
  const heading = Math.random() * Math.PI * 2;

  const card = {
    data,
    el,
    naturalW: loaded.naturalWidth,
    naturalH: loaded.naturalHeight,
    x: 0,
    y: 0,
    w: 0,
    h: 0,
...
function onPointerDown(card, event) {
  if (snapping || modal.hidden === false) return;
  ...
  card.grabX = event.clientX - rect.left;
  card.grabY = event.clientY - rect.top;
  card.x = rect.left;
  card.y = rect.top;
}
...
function onPointerMove(event) {
  ...
  event.preventDefault();
  dragging.x = event.clientX - dragging.grabX;
  dragging.y = event.clientY - dragging.grabY;
  dragging.x = Math.max(-40, Math.min(window.innerWidth - dragging.w + 40, dragging.x));
  dragging.y = Math.max(-40, Math.min(window.innerHeight - dragging.h + 40, dragging.y));
...
spawnPetals();
Promise.all(MEMORIES.map((memory, index) => createCard(memory, index))).then(() => {
  requestAnimationFrame(tick);
});
